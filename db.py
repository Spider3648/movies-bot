...
our_url = "https://s-s.lordfilms-s.biz/films/"
url = our_url
...
while x <= 1:
    ...
    for i in range(len(opisanie_list)):
        name1 = name_list[i]
        god1 = god_list[i]
        opisanie1 = opisanie_list[i]
        link2 = l[i]
        print(link2)
        cur.execute("""INSERT INTO Triangle_Kino (RESURS, NAME, GOD, OPISANIE, LINK_STR) VALUES (?, ?, ?, ?, ?);""",
                    (resursZERO, name1, god1, opisanie1, link2))
        db.commit()
        print(str(round((i + 1) * 100/ len(opisanie_list), 2)) + "%")

    x = x + 1

    url = our_url + "page/" + str(x) + "/"
